...
/**
 * Glucose block class
 * <p>Behaves the same as powdered snow besides some behaviour</p>
 */
public class GlucoseBlock extends Block {
    private static final VoxelShape FALLING_SHAPE = VoxelShapes.cuboid(0.0, 0.0, 0.0, 1.0, 0.9f, 1.0);

    /**
     * Constructs block with passed parameters
     * @param settings settings
     */
    public GlucoseBlock(Settings settings) {
        super(settings);
    }
     ...
     * <p>Slows the player, plays particle effects and extinguishes them</p>
     * @param state the glucose blocks BlockState
        ...
        if (!(entity instanceof LivingEntity) || entity.getBlockStateAtPos().isOf(this)) {
            ...
            if (world.isClient) {
                ...
                if (bl && random.nextBoolean()) {
                    world.addParticle(ParticleTypes.ITEM_SLIME, entity.getX(), pos.getY() + 1, entity.getZ(), MathHelper.nextBetween(random, -1.0f, 1.0f) * 0.083333336f, 0.05f, MathHelper.nextBetween(random, -1.0f, 1.0f) * 0.083333336f);
                }
            }
        }
